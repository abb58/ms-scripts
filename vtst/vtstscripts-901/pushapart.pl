...
################################################
# Sometimes when modifing density of a liquid atoms can be too close to eachother.
# This script applies a repelling force between each pair
# and pushes them apart until the minimum distance is reached. 
# Created by Daniel Sheppard from nebaviod.pl 9.17.2014 
################################################
...
if(@ARGV>0) {
    ...
    die "Usage: pushapart.pl distance(in Ang) file(POSCAR format) \n";
}
$file = $ARGV[1];
if(!(-e $file)) { die "file $file does not exist.\n"; }
$ORTHOGONAL = 0;
if(@ARGV>2) { $ORTHOGONAL = $ARGV[2]; }

print "working on $file...\n";
($coordinates,$basis,$lattice,$num_atom,$total_atoms,$selectiveflag,$selective,$description) = read_poscar($file);
...
write_poscar($coordinates,$basis,$lattice,$num_atom,$total_atoms,$selectiveflag,$selective,$description,$file);
